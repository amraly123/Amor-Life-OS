...
const TaskManager: React.FC<TaskManagerProps> = ({ tasks, setTasks, goals }) => {
  ...
  const toggleSubTask = (taskId: string, subId: string) => {
    setTasks(prev => prev.map(t => {
      if (t.id === taskId) {
        ...
        // If all subtasks done, don't auto-complete main task (let user decide), but show progress
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      {task.important && <span className="p-1 px-2 bg-amber-50 text-amber-600 rounded-lg text-[10px] font-bold flex items-center gap-1"><Star size={10} fill="currentColor" /> هام</span>}
                      {task.urgent && <span className="p-1 px-2 bg-red-50 text-red-600 rounded-lg text-[10px] font-bold flex items-center gap-1"><AlertCircle size={10} /> عاجل</span>}
                    </div>
                      ...
                      <div className="mt-5 space-y-3 pt-4 border-t border-slate-50 animate-in fade-in zoom-in duration-200">
                        ...
                        {task.subtasks && task.subtasks.map(sub => (
                          <div key={sub.id} className="flex items-center gap-3 group/sub">
                            ...
                            <span className={`text-xs font-bold flex-1 ${sub.completed ? 'text-slate-300 line-through' : 'text-slate-600'}`}>{sub.title}</span>
                            <button onClick={() => removeSubTask(task.id, sub.id)} className="opacity-0 group-hover/sub:opacity-100 transition-opacity"><Trash2 size={12} className="text-slate-200 hover:text-red-400" /></button>
                          </div>
                        ))}
  ...
  return (
    <div className="space-y-6">
      {/* Header & Stats - Keep as is but with refined styling */}
      ...
      {/* Add Task Form - Premium Glass Style */}
                        ...
                        <div className="space-y-4">
                          {task.subtasks && task.subtasks.map(sub => (
                            <div key={sub.id} className="flex items-center gap-4 bg-white p-3 rounded-2xl shadow-sm border border-slate-100 group/sub">
                              ...
                              <span className={`text-sm font-bold flex-1 ${sub.completed ? 'text-slate-300 line-through' : 'text-slate-700'}`}>{sub.title}</span>
                              <button onClick={() => removeSubTask(task.id, sub.id)} className="text-slate-200 hover:text-red-400 opacity-0 group-hover/sub:opacity-100 transition-opacity"><Trash2 size={14} /></button>
                            </div>
                          ))}
