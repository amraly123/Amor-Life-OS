    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2">
          <span className="px-3 py-1 bg-sky-100 text-sky-700 rounded-full text-xs font-bold">الكل (٨)</span>
        ...
        <button className="flex items-center gap-2 bg-sky-500 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-lg shadow-sky-500/20">
          <Plus size={18} />
          هدف جديد (OKR)
        </button>
      </div>
        ...
        {goals.map((goal) => (
          <div key={goal.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-start mb-4">
              <div>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase mb-2 inline-block ${goal.category === 'Work' ? 'bg-sky-100 text-sky-600' : 'bg-indigo-100 text-indigo-600'
                  }`}>
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-black text-sky-500">{goal.progress}%</span>
              </div>
