  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative group overflow-hidden rounded-[3rem] bg-slate-900 p-1">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/20 via-blue-500/20 to-purple-500/20 opacity-50 group-hover:opacity-100 transition-opacity duration-1000"></div>
        ...
        <div className="relative bg-slate-900 rounded-[2.8rem] p-8 md:p-10 border border-white/5 overflow-hidden">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="relative">
               <div className="w-24 h-24 md:w-32 md:h-32 bg-gradient-to-tr from-emerald-500 to-teal-400 rounded-3xl flex items-center justify-center shadow-2xl shadow-emerald-500/40 rotate-3 group-hover:rotate-6 transition-transform">
            ...
            <div className="flex-1 text-center md:text-right">
              <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                <span className="text-emerald-400 text-xs font-black uppercase tracking-[0.2em]">انتصار الأسبوع الاستراتيجي</span>
            ...
            <button className="bg-white text-slate-900 px-8 py-4 rounded-2xl font-black text-sm uppercase flex items-center gap-2 hover:bg-emerald-500 hover:text-white transition-all shadow-xl active:scale-95">
               تحديث التقدم
               <ChevronLeft size={18} />
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {Object.values(LifeRole).map((role, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all text-center group cursor-pointer">
               <div className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:bg-emerald-50 transition-colors">
                  <Target size={20} className="text-slate-400 group-hover:text-emerald-500" />
               </div>
               <span className="font-bold text-slate-700 block">{role}</span>
               <span className="text-[10px] text-emerald-500 font-bold uppercase mt-1 block">إتقان: ٨٥٪</span>
            </div>
          ))}
        </div>
      </div>
